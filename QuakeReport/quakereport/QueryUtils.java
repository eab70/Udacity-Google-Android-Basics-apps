...
public final class QueryUtils {
    ...
    // Return a list of Earthquake objects that has been built up from parsing the JSON response.
    public static ArrayList<Earthquake> extractEarthquakes() {

        // Create an empty ArrayList to add the individual earthquakes to
        ArrayList<Earthquake> earthquakes = new ArrayList<>();

        // Parse the SAMPLE_JSON_RESPONSE
        try {
        JSONObject jSON_results = new JSONObject(SAMPLE_JSON_RESPONSE);
        // Getting JSON Array node
        JSONArray earthquakesJ = jSON_results.getJSONArray("features");

        // loop through All earthquakes
        for (int i = 0; i < earthquakesJ.length(); i++) {
            JSONObject currentE = earthquakesJ.getJSONObject(i);
            JSONObject p = currentE.getJSONObject("properties");
        ...
        // If an error is thrown above, catch the exception here so the
        // app doesn't crash, and print a log message with the message from the exception.
